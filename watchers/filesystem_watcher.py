...
# Allow running from any directory (fixes ModuleNotFoundError for base_watcher)
sys.path.insert(0, str(Path(__file__).parent))
# Allow importing shared utils from vault root
...
class FileSystemWatcher(BaseWatcher):
    ...
    def __init__(
        ...
        # Audit logger
        self.audit = AuditLogger(str(self.vault_path))
    ...
    def create_action_file(self, item: Path) -> Path:
        ...
        self.audit.log(
            ...
            target=str(item),
            details={"filename": item.name, "extension": item.suffix.lower()},
            status="success",
        )
...
"""
        ...
        if self.dry_run:
            ...
            print("-" * 40)
            self.audit.log(
                action="action_file_created",
                component="filesystem_watcher",
                target=action_filename,
                details={"source_file": item.name, "file_type": file_type, "dry_run": True},
                status="skipped",
